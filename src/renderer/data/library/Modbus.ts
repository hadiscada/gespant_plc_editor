...
const ModbusVariableSchema = BaseLibraryVariableSchema

const ModbusPouSchema = BaseLibraryPouSchema.extend({
  variables: z.array(ModbusVariableSchema),
})

export const ModbusLibrarySchema = BaseLibrarySchema.extend({
  pous: z.array(ModbusPouSchema),
})

type ModbusLibrary = z.infer<typeof ModbusLibrarySchema>

const Modbus: ModbusLibrary = {
  name: 'Modbus',
  version: '1.0.0',
  author: 'Autonomy Logic',
  stPath: 'src/renderer/data/library/Modbus/st',
  cPath: 'src/renderer/data/library/Modbus/c',
  pous: [
    {
      name: 'TCP_CONNECT',
      type: 'function-block',
